...
<!-- Modal Alihkan / Pindahkan Ruangan oleh Admin -->
<div id="relocateBookingModal" class="responsive-modal fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm hidden items-center justify-center">
    <div class="responsive-modal-panel bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full p-4 sm:p-6 shadow-2xl border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-150 space-y-4">
        <div class="flex items-start justify-between pb-3 border-b border-slate-100 dark:border-slate-700">
            <div>
                <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300">
                    Operasional Ruangan
                </span>
                <h3 class="text-base font-bold text-slate-900 dark:text-white mt-1">Alihkan ke Ruangan Lain</h3>
            </div>
            <button type="button" onclick="closeRelocateModal()" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 rounded-lg">
                <i class="fas fa-times text-lg"></i>
            </button>
        </div>
        ...
        <form method="POST" action="admin_bookings.php" class="space-y-4">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="action" value="relocate_room">
            <input type="hidden" name="booking_id" id="relocateModalBookingId" value="">

            <div class="p-3 bg-slate-50 dark:bg-slate-900/70 rounded-xl border border-slate-200/80 dark:border-slate-800 text-xs">
                <div class="font-bold text-slate-800 dark:text-slate-200 text-sm" id="relocateModalTitle"></div>
                <div class="text-slate-500 dark:text-slate-400 mt-1" id="relocateModalInfo"></div>
            </div>

            <div>
                <label for="relocateNewRoomSelect" class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                    Pilih Ruangan Pengganti (Tujuan) <span class="text-amber-500">*</span>
                </label>
                <select 
                    name="new_room_id" 
                    id="relocateNewRoomSelect" 
                    required 
                    class="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500 transition cursor-pointer"
                >
                    <option value="">-- Pilih Ruangan Pengganti --</option>
                    <?php if (!empty($rooms)): ?>
                        <?php foreach ($rooms as $rm): ?>
                            <option value="<?php echo (int) $rm['id']; ?>">
                                <?php echo htmlspecialchars($rm['name']); ?> (Kapasitas: <?php echo (int) $rm['capacity']; ?> org - <?php echo htmlspecialchars($rm['location']); ?>)
                            </option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
            ...
            <div>
                <label for="relocateModalReason" class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                    Alasan / Keterangan Pengalihan <span class="text-amber-500">*</span>
                </label>
                <textarea 
                    name="relocate_reason" 
                    id="relocateModalReason" 
                    rows="3" 
                    required 
                    placeholder="Contoh: Terjadi kebocoran AC di ruangan sebelumnya sehingga dialihkan ke ruangan alternatif."
                    class="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500 transition resize-none"
                ></textarea>
                <p class="text-[11px] text-slate-400 mt-1">Keterangan ini akan tampil pada status pemesanan pengguna serta jadwal ruangan baru.</p>
            </div>

            <div class="pt-3 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-2">
                <button type="button" onclick="closeRelocateModal()" class="px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-bold rounded-xl text-xs transition">
                    Batal
                </button>
                <button type="submit" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl text-xs shadow-md shadow-amber-500/20 transition flex items-center gap-1.5">
                    <i class="fas fa-arrows-split-up-and-left"></i> Simpan & Alihkan Ruangan
                </button>
            </div>
        </form>
    </div>
